...
def psf2otf(psf, h, w):
    ...
    h_psf = psf_shape[0]
    w_psf = psf_shape[1]
    psfNormalpad = F.pad(psfNormal, pad=(int(np.ceil((h-h_psf)/ 2)), int(np.floor((h-h_psf) / 2)), int(np.ceil((w-w_psf) / 2)), int(np.floor((w-w_psf) / 2))), mode='constant', value=0)
...
def deconvolve_wnr(blur, psf, gamma):
    ...
    wiener_filter_temp = torch.conj(otf) / (torch.abs(otf) ** 2 + gamma)
    wiener_filter_temp = torch.unsqueeze(wiener_filter_temp, dim=0)
    wiener_filter_temp = torch.unsqueeze(wiener_filter_temp, dim=0)
    if img_shape[0] == 2:
...
class FOVBlock(nn.Module):
    ...
    def __init__(self, input_channel=3, output_channel=32, nf=32):
        super().__init__()
        # spatial attention
        self.sAtt_fov_1 = nn.Conv2d(input_channel-1, nf, 3, 1, 1, bias=True)      # 输入两个视场通道，输出32个通道，卷积和3X3，stride=1, padding=1
