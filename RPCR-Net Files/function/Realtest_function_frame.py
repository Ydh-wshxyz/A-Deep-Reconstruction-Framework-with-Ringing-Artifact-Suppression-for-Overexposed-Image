...
from Dataset import dataset_load_RealImage3, PSF_load
import torch
import numpy as np
...
def test_function(args):
    ...
    # 加载FOV Data
    FOVX = sio.loadmat('G:/02.VIS/02.Python Project/Pytorch_PSFFOVKPN_3X3Split_weight/X.mat')
    FOVY = sio.loadmat('G:/02.VIS/02.Python Project/Pytorch_PSFFOVKPN_3X3Split_weight/Y.mat')
    ...
    for frame in range(20):
        ...
        # Dataset
        test_data, test_number = dataset_load_RealImage3(args.test_dir, 'mat', 2)  # 测试数据
        for ind in range(test_number):
            image_out = np.squeeze(test_data[ind])
            img_test = test_data[ind].copy() / 65535. * 0.8
            ...
            input_out_binning = average_pool(image_out, 2)
            ...
            save_dir = args.recovery_dir + '/%02d' % ind
            if not os.path.exists(save_dir):
                os.makedirs(save_dir)

            model_output_mat_name = save_dir + '/blurred%05d.mat' % frame
            sio.savemat(model_output_mat_name, {'data': input_out_binning.astype(np.uint16)})
            model_output_mat_name = save_dir + '/deblurred%05d.mat' % frame
            sio.savemat(model_output_mat_name, {'data': net_output_binning.astype(np.uint16)})
            model_output_mat_name = save_dir + '/wnrdeblurred%05d.mat' % frame
